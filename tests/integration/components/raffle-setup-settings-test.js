...
describe('Integration | Component | raffle setup settings', function() {
  ...
  it('has a header', function() {
    render.call(this);
    expect(find('[data-test-header]').textContent.trim()).to.equal("What're the specifics of this raffle?");
  });
  ...
  describe('name input', function() {
    it('shows the existing raffle name initially', function() {
      ...
      expect(find('[data-test-raffle-name-input]').value).to.equal('A very special raffle');
    });

    it('mutates the raffle name when changed', async function() {
      render.call(this);
      await fillIn('[data-test-raffle-name-input]', 'A very average raffle');
      expect(find('[data-test-raffle-name-input]').value).to.equal('A very average raffle');
  ...
  describe('number of winners input', function() {
    it('is a number input field', function() {
      render.call(this);
      expect(find(`${'[data-test-raffle-num-winners-input]'}[type='number']`)).to.be.ok;
    });
    ...
    it('has a singular label when the field is 1', function() {
      this.set('raffle', make('raffle', { numberOfWinners: 2 }));
      render.call(this);
      expect(find('[data-test-raffle-num-winners-label]').textContent.trim()).to.equal('winners');
    });

    it('has a pluralized label when the field is greater than 1', function() {
      this.set('raffle', make('raffle', { numberOfWinners: 1 }));
      render.call(this);
      expect(find('[data-test-raffle-num-winners-label]').textContent.trim()).to.equal('winner');
    });
  });
